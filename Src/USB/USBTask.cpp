...
void USB_Receive_Data(void) {
	...
	if (UsbRxFrame.frame->startByte == 0xff && UsbRxFrame.frame->code == 0x40 && UsbRxFrame.frame->length == USB_RXFRAME_SIZE - 4
		...
		// potrzebny w�tek na servo i silnik uwzgl�dniaj�cy pochodne sterowania pomi�dzy instrukcjami.
		...
		left_indicator = (uint8_t)(UsbRxFrame.frame->left_indicator_u);
		right_indicator = (uint8_t)(UsbRxFrame.frame->right_indicator_u);
		timestamp = (uint32_t)(UsbRxFrame.frame->timecode);
	}
}

//#define USB_TXFRAME_SIZE 38
//#define USB_TXFRAME_SIZE 39
...
//	if(start_obstacle_USB)
//		start_obstacle_sent = 1;
}
void USB_SetTransmission(void)
{
